...
begin
    ...
    begin
        ...
        if filtration_end_t < tspan[2]
            ...
            annotate!([(filtration_end_t: filtration_end_t, 1.5, text("отключение\nфильтрации",9))])
        end
    ...
    for i in [10,length(U_set)] #eachindex(U_set)
        reduce=Int(round(0.00*length(x)))
        plot_1 = plot(
            x[reduce+1:end-reduce],
            circshift(abs.(U_set[i])[reduce+1:end-reduce],150);
        ...
        plot!(
            x[reduce+1:end-reduce],
            abs.(initial_function.(x))[reduce+1:end-reduce];
        ...
        plot!(
            x[reduce+1:end-reduce],
            circshift(abs.(possible_NSE_3_5_solution[i])[reduce+1:end-reduce],150);
    ...
    #= begin
        ...
        annotate!(
            [(
                ...
                text("Рассеяно $(
                    round(
                        ((I1+I1_dissipated)[end] - I1[end])/(I1+I1_dissipated)[end] * 100,digits=5)
                ) % от " * L"I_{1}",9)
            )]
        )
        xlabel!("t")
    ...
    begin
        ...
        annotate!(
            [(
                ...
                text("Рассеяно $(round(((I2+I2_dissipated)[end] - I2[end])/(I2+I2_dissipated)[end] * 100,digits=5)) % от "*L"I_{2}",9)
            )]
        )
        xlabel!("t")
    ...
    if !isnothing(pulse_maximum)
        ...
        xlabel!("t")
        ylabel!(L"\max|U^{m}|")
        savefig(plot_4, "run/plots/pike_behaviour.png")
