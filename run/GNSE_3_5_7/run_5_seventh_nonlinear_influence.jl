# include("run/run_5_seventh_nonlinear_influence.jl")
...
tspan = (0.0,1870.0)
xspan = (-80.0, 80.0)
capture_times = [] #collect(0.0:2.0:tspan[2])#[10.0, 30.0, tspan[2]]

initial_function = (x) -> NSE_3_soliton(x, 0.0, k, ω, theta_0, z_0)

filtration_factor, filtration_end_t = 1.02, 550.0
...
x, t, U_set, (I1_dissipated, I2_dissipated), pulse_maximum, (I1, I2) = @profile cuda_solve(
    tspan,
    xspan,
    tau,
    h,
...
begin
    way = "nefull"
    index = way == "full" ? eachindex(U_set) : [length(U_set)]
    # Вычисление ошибок
    #= begin
        true_tolerance = Vector{Float64}([])
        possible_NSE_3_5_7_solution = Vector{Vector{Float64}}(undef, length(U_set))
        for i in index
            possible_NSE_3_5_7_solution[i] = abs.(
                construct_best_approximate_NSE_3_5_7_solution(
                    x,
                    h,
                    abs.(U_set[i]),
                    xspan[2]-xspan[1],
            ...
            push!(true_tolerance, relative_error_to_amplitude(possible_NSE_3_5_7_solution[i],U_set[i]))
        end
    ...
    #= begin
        ...
        savefig(plot_0, "run/plots/run_5/true_tolerance.png")
    end =#
    ...
    for i in index
        ...
        plot_1 = plot(
            ...
            circshift(abs.(U_set[i]),n_circshift)[n_reduce+1:end-n_reduce];
        ...
        #= plot!(
            x[n_reduce+1:end-n_reduce],
            circshift(
                100 .* (abs.(U_set[i])[n_reduce+1:end-n_reduce] .- abs.(possible_NSE_3_5_7_solution[i])[n_reduce+1:end-n_reduce]),
                n_circshift
            );
        ...
        #= plot!(
            x[n_reduce+1:end-n_reduce],
            (circshift(abs.(possible_NSE_3_5_7_solution[i]),n_circshift))[n_reduce+1:end-n_reduce];
        ...
        xlabel!("x")
        ylabel!("|U|")
        savefig(plot_1, "run/plots/run_5/profiles_T=$(capture_times[i]).png")
    end
    ...
    begin
        ...
        annotate!(
            [(
                ...
                (maximum(I1) - minimum(I1))*0.95+minimum(I1),
                text("Рассеяно $(
                    round(
        ...
        savefig(plot_2, "run/plots/run_5/I1_influenced.png")
    end
    ...
    begin
        ...
        annotate!(
            [(
                (tspan[2]-tspan[1])/1.5 + tspan[1],
                (maximum(I2) - minimum(I2))*0.95+minimum(I2),
        ...
        savefig(plot_3, "run/plots/run_5/I2_influenced.png")
    end
    ...
    if !isnothing(pulse_maximum)
        plot_4 = plot(
            t,
            smooth_vector(pulse_maximum,300); 
            line=(:path,:solid,:black,1), 
            legend = false,
            dpi=800,
            tickfontsize=10,
        ...
        #= annotate!(
            [(
                t[end-Int(round(tspan[2]/15/tau))],
                (maximum(pulse_maximum)-minimum(pulse_maximum))*0.05+mean(pulse_maximum[end-100:end]),
                text(" δ= $(
                    round((maximum(pulse_maximum[t2_index:end])-minimum(pulse_maximum[t2_index:end]))/maximum(pulse_maximum[t2_index:end]) *100,digits=5)
                    ) %",9)
            )]
        ) =#
        xlabel!("t")
        ylabel!(L"\max|U^{m}|")
        savefig(plot_4, "run/plots/run_5/pike_behaviour.png")

        time_gap=100.0
        time_gap < tspan[2] || @error "time_gap при построении поведения пика импльуса слишком большой"
        ...
        xlabel!("t")
        ylabel!(L"\max|U^{m}|")
        savefig(plot_5, "run/plots/run_5/pike_behaviour_initial.png")
        ...
        xlabel!("t")
        ylabel!(L"\max|U^{m}|")
        savefig(plot_6, "run/plots/run_5/pike_behaviour_final.png") =#
    end
